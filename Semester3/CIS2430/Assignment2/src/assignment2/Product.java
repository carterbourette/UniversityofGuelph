...
public abstract class Product {
    ...
    public Product(String pID, String nm, String p, String y) throws Exception {
        if (pID.length() != 6 || (!isInteger(pID))) throw new Exception("Invalid product id.");
        if(nm.equals("")) throw new Exception("Invalid name.");
        if(Validator.parseInt(p)<= 0) throw new Exception("Invalid price.");
        int n = Validator.parseInt(y);
        if(n < 1000 || n > 9999) throw new Exception("Invalid price.");
        this.productID = pID;
        this.name = nm;
        this.price = p;
        this.year = y;
